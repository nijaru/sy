    ...
    R: AsyncRead + Unpin,
{
    ...
    // Length is validated before allocation, so hostile peers cannot request an
    // unbounded buffer through the wire header.
...
mod tests {
    ...
    proptest! {
        #[test]
        fn arbitrary_headers_never_panic(header in any::<[u8; HEADER_LEN]>()) {
