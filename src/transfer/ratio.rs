//! Change-ratio sampling for transfer strategy selection.
//!
//! Moved from the condemned `crate::delta` stack (item 7): this sampling
//! heuristic is the only piece of the legacy delta implementation with a live
//! caller (`endpoint::transfer::reflink_patch`), so it now lives with the v3
//! transfer layer instead of blocking `src/delta/` deletion.
